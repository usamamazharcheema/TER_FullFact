        ...
        if claim:
            ...
            if autresClaims:
                ...
                for row in autresClaims:
                    sclaim = row.find('div', {"class": "card-body card-claim-body"})
                    if sclaim:
                        summarized_claim = sclaim.get_text().replace("\nWhat was claimed\n", "")
                        claimReview.append(summarized_claim)

                if ind_claim_list != "empty":
                    ind = 0
                    while ind < len(ind_claim_list):
                        claimReview.append(ind_claim_list[ind])
                        ind = ind + 2
                ...
                for claimReview_Reviewed in claimReview:

                    dic_data = additionalRowsKg.briefAdditionalRowsKg(claimReview_Reviewed, url, t,
                                                                  )
                    if dic_data != "empty":
...
def get_all_claimskg(criteria):
    ...
    # print("Articles having more than one summarized claim and more individual claims than summarized claims: "+ str(article_indClaim_gt_Sclaim))
