...
def exactractionClaim(page, url, maxClaims):
    global urlTraite, urls_, claims, idClaim, uriSansClaim, nbClaims, count_url, article_more_than_one_Sclaim, article_indClaim_gt_Sclaim, dic_data
        ...
        if claim:
            ...
            if autresClaims:
                    ...
                    if (len(ind_claim_list) / 2 == 1) or (len(autresClaims) == 1):
                        ...
                        while i < len(ind_claim_list):
                            for row in autresClaims:
                                dic_data = additionalRows.briefAdditionalRows(row, result, url, t, liensRevue, d,
                                                                              ind_claim_list[i], ind_claim_list[i + 1])
                                if dic_data != "empty":
                                    print(dic_data.getDict())
                                    claims.append(dic_data.getDict())
                            i = i + 2
                        ...
                        for row in autresClaims:
                            ...
                            while j < len(ind_claim_list):
                                dic_data = additionalRows.briefAdditionalRows(row, result, url, t, liensRevue, d,
                                                                              ind_claim_list[j], ind_claim_list[j + 1])
                                if dic_data != "empty":
                                    similarities = dic_data.getDict()['similarity']
                                    simi_list1.append(similarities)
                                    data_simi1.append(dic_data.getDict())
                                j = j + 2
                            # check_duplicates=[]
                            for ind_simi in data_simi1:
                                if ind_simi['similarity'] == max(simi_list1):
                        ...
                        if len(ind_claim_list) / 2 > len(autresClaims):
                            ...
                            while k < len(ind_claim_list):
                                ...
                                if not any(ind_claim_list[k] == claims_value['ind_claim'] for claims_value in claims):
                                    ...
                                    data_simi2 = []
                                    for row in autresClaims:
                                        dic_data2 = additionalRows.briefAdditionalRows(row, result, url, t,
                                                                                       liensRevue, d,
                                                                                       ind_claim_list[k],
                                                                                       ind_claim_list[k + 1])
                                        if dic_data2 != "empty":
                                            similarities = dic_data2.getDict()['similarity']
                                            simi_list2.append(similarities)
                                            data_simi2.append(dic_data2.getDict())

                                    for ind_simi in data_simi2:
                                        if ind_simi['similarity'] == max(simi_list2):
                                            claims.append(ind_simi)
                                            print(ind_simi)
                                k = k + 2
                ...
                # If Individual claim is empty
                    ...
                    for row in autresClaims:
                        c_dic = additionalRows.briefAdditionalRows(row, result, url, t, liensRevue, d,
                                                                   "empty", "empty")
                        print("without Ind_claim")
                        if c_dic != "empty":
                            print(c_dic.getDict())
                            claims.append(c_dic.getDict())
